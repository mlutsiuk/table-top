...
> `#` читає параметр виконання дії. Поле `mechanic` у прикладах нижче теж застаріле:
> механік як плагінів немає (ADR-013), операція адресуватиме ключ `TraitDef`. Приклади
> писалися до цих рішень і будуть приведені до них разом з реалізацією Етапу 5.
