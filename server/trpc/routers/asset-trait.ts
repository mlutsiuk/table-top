...
export const assetTraitRouter = router({
  ...
  attach: privateProcedure
    .input(z.object({ assetId: z.uuid(), mechanicId: z.uuid() }))
    .mutation(({ input, ctx }) => ctx.assetTraits.attach(input.assetId, input.mechanicId)),

  /** The whole trait, because that is the unit its mechanic validates. */
